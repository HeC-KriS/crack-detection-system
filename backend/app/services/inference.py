...
class YOLOService:
    ...
    def load(self) -> None:
        ...
        logger.info("Loading YOLO model from %s on device=%s", self._model_path, self._device)
    ...
    def infer(self, frame_bgr: np.ndarray, meta: FrameMeta) -> InferenceResult:
        ...
        if result.masks is not None:
            for i, mask in enumerate(result.masks):
                cls_id = int(result.boxes.cls[i].item()) if result.boxes else 0
        ...
        # ── Threshold filter ────────────────────────────────────────────────
        filtered_detections = [d for d in detections if d.confidence >= meta.alert_threshold]
        filtered_segments = [s for s in segmentations if s.confidence >= meta.alert_threshold]

        max_confidence = (
            max((d.confidence for d in filtered_detections), default=0.0)
        )
